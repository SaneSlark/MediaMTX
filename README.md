...
延迟转发会同时转发视频和音频：视频支持 H.264 和 H.265，音频支持 PCMA、PCMU、AAC（MP4A-LATM / MPEG4-GENERIC）和 Opus；其它编码的流会被忽略并打印日志。新增或修改摄像头配对后，需要执行 `docker compose restart mediamtx` 重新读取配置。

断线默认每 30 秒重试；持续 30 秒无视频会触发重连。如果 GStreamer 清理超过 15 秒仍未结束，脚本退出，由 Compose 的重启策略恢复整个容器，其他摄像头也会短暂中断。直接运行脚本时，需要自行配置进程自动重启。
